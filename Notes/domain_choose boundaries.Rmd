---
title: "Choosing StrathE2E domain extent - NOMAR"
...
For the NOrth Mid-Atlantic Ridge we have inspected the distribution of vertical diffusivity values in the region from NEMO-MEDUSA model output. 60 m looks like a sensible depth to separate the shallow and deep layers.

**We updated this too 100 m to capture the deeper chlorophyll for the region, according to https://doi.org/10.1016/j.seares.2013.04.016.** 
...
Finally we need to "cut" the polygons defined by depths and distances from shore so that they don't extend around the whole world. Our first suggestion is to use the EEZ for the Azores. If there are reasons to choose different breaks, please let us know.
...
The model domain is shown in grey with white borders, while the fishing effort is shown as a heatmap on a log scale. By using the EEZ to define the model domain, we appear to capture all relevant fishing activity. Is the entire EEZ of interest to the case study? If not, please make some suggestions for altering the extent of the offshore zone. 
